...
class ExtendedFingerprint_viz(Fingerprint_viz):
    def __init__(self, radius, T, input_feature_dim, input_bond_dim, fingerprint_dim, output_units_num, p_dropout, physicochemical_feature_dim):
        ...
        self.physicochemical_fc = nn.Linear(physicochemical_feature_dim, 256)
        self.physicochemical_bn = nn.BatchNorm1d(256)
        self.physicochemical_fc2 = nn.Linear(256, fingerprint_dim)
        ...
        #self.final2_fc = nn.Linear(1024, 512)
        self.final3_fc = nn.Linear(256, 64)
        self.bn_final3 = nn.BatchNorm1d(64) # Added BatchNorm Layer
        self.final4_fc = nn.Linear(64, output_units_num)

    def forward(self, atom_list, bond_list, atom_degree_list, bond_degree_list, atom_mask, physicochemical_features):
        atom_feature_viz, atom_attention_weight_viz, mol_feature_viz, mol_feature_unbounded_viz, mol_attention_weight_viz, mol_prediction = super().forward(atom_list, bond_list, atom_degree_list, bond_degree_list, atom_mask)
        ...
        # 1024 - 512
        #final4_prediction = self.final2_fc(final3_prediction)
        #final5_prediction = self.dropout(final4_prediction)
        #final6_prediction = F.relu(final5_prediction)
        # 512 - 64
 
        final7_prediction = F.relu(self.bn_final3(self.final3_fc(final2_prediction))) # Applied BatchNorm Layer

        final8_prediction = self.dropout(final7_prediction)
        # 64 - 2
        final10_prediction = self.final4_fc(final8_prediction)
        final10_prediction = F.sigmoid(final10_prediction)
      
        return atom_feature_viz, atom_attention_weight_viz, mol_feature_viz, mol_feature_unbounded_viz, mol_attention_weight_viz, final10_prediction
